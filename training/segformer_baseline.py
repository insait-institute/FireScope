...
from models.segformer import SegformerNoFilm
...
def build_model_and_device(local_rank: int):
    ...
    # keep the whole model in train mode, but put the encoder in eval()
    # to disable dropout / stochastic depth inside the backbone
    model.train()
    model.encoder.eval()
    return model, device
...
def build_cfg_from_args():
    ...
    p.add_argument('--out_dir', type=str, default='/work/wildfirerisk/trainings/small/segformer_baseline')
