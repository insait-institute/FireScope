...
from models.unet import Unet, FilmUnet
from models.segformer import FilmSegformer
...
def build_model_and_device(local_rank: int):
    ...
    model = FilmSegformer(cond_dim=1)
...
def evaluate(model, device, val_loader, ssim, huber, cfg: Cfg) -> Dict[str, float]:
    ...
    with torch.no_grad():
        for img, climate, rast, cheat, base in val_loader:
            ...
            cond = cheat.to(device, non_blocking=True)
...
def save_val_samples_fixed(model, device, val_dataset: CustomDataset, indices: List[int], out_dir: str, epoch: int):
    ...
    with torch.no_grad():
        for i, idx in enumerate(indices, start=1):
            ...
            cond = cheat.unsqueeze(0).to(device)
            rast = rast_t.unsqueeze(0).to(device)
            pred = model(img, cond=cond)  # [-1,1]
...
def train(cfg: Cfg):
    ...
    # Datasets / loaders
    train_ds = CustomDataset(cfg.train_img_dir, cfg.train_rast_dir, climate_dir=cfg.climate_json, with_cheating=True, vlm_pred_json="/work/wildfirerisk/vlm_cot_big_checkpoint_4800.json")
    val_ds   = CustomDataset(cfg.val_img_dir, cfg.val_rast_dir, climate_dir=cfg.climate_json, with_cheating=True, vlm_pred_json="/work/wildfirerisk/vlm_cot_big_checkpoint_4800.json")
    ...
    with open(os.path.join(cfg.out_dir, 'logs.out'), 'w') as f:
        for epoch in range(cfg.epochs):
            ...
            scheduler.step()
            if is_main_process():
                print(f"Epoch {epoch+1} done in {time.time()-t0:.1f}s; ckpt: {ckpt_path}")
...
def build_cfg_from_args():
    ...
    p.add_argument('--train_img_dir', type=str, default='/work/wildfirerisk/large_dataset/split_satellite_images/train')
    p.add_argument('--train_rast_dir', type=str, default='/work/wildfirerisk/large_dataset/normalised_risk_rasters/train')
    p.add_argument('--val_img_dir',   type=str, default='/work/wildfirerisk/small_dataset/satellite_images/val')
    p.add_argument('--val_rast_dir',  type=str, default='/work/wildfirerisk/small_dataset/normalised_risk_rasters/val')
    p.add_argument('--climate_json',  type=str, default='/work//wildfirerisk/climate_data.json')
    p.add_argument('--out_dir', type=str, default='/work/wildfirerisk/trainings/big/oracle_segformer')
    ...
    p.add_argument('--log_every', type=int, default=5.0)
