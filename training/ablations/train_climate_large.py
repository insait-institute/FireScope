...
def train(cfg: Cfg):
    ...
    # Datasets / loaders
    train_ds = CustomDataset(cfg.train_img_dir, cfg.train_rast_dir, climate_dir=cfg.climate_json, with_cheating=False, train_img_dir=cfg.train_img_dir)
    val_ds   = CustomDataset(cfg.val_img_dir, cfg.val_rast_dir, climate_dir=cfg.climate_json, with_cheating=False, train_img_dir=cfg.train_img_dir)
        ...
        for it, (img, climate, rast, cheat, base) in enumerate(train_dl):
            ...
            if is_main_process() and (it+1) % cfg.log_every == 0:
                ...
                print(f"[epoch {epoch+1:03d}] it {it+1:04d}/{len(train_dl)} "
                    f"recon={run_recon/nlog:.4f} ssim={run_ssim/nlog:.4f} grad={run_grad/nlog:.4f}")
                run_recon = run_ssim = run_grad = 0.0

        # ---- Eval ----
        if epoch % cfg.eval_every == 0:
            val_metrics = evaluate(model, device, val_dl, ssim, huber, cfg)
            print(f"[val @ epoch {epoch+1:03d}] recon={val_metrics['recon']:.4f} ssim={val_metrics['ssim']:.4f} grad={val_metrics['grad']:.4f}")
            if is_main_process():
                pass
...
def build_cfg_from_args():
    ...
    p.add_argument('--train_img_dir', type=str, default='/work/wildfirerisk/large_dataset/satellite_images_full/train')
    p.add_argument('--train_rast_dir', type=str, default='/work/wildfirerisk/large_dataset/normalised_risk_rasters/train')
    p.add_argument('--val_img_dir',   type=str, default='/work/wildfirerisk/large_dataset/satellite_images/val')
    p.add_argument('--val_rast_dir',  type=str, default='/work/wildfirerisk/large_dataset/normalised_risk_rasters/val')
    p.add_argument('--climate_json',  type=str, default='/work/wildfirerisk/large_dataset/climate_data.json')
    p.add_argument('--out_dir', type=str, default='/work/wildfirerisk/trainings/big/unet_climate')
