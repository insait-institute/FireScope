...
from models.sam import SamFiLMTransformerDecoder
...
def build_model_and_device(local_rank: int):
    ...
    model = QwenVLTransformerRaster(
        model_id="Qwen/Qwen2.5-VL-7B-Instruct",
        out_ch=1,
        d_model=512, depth=8, heads=8, grid_hw=(43,43),
        dtype=torch.bfloat16,
        freeze_qwen=False,   # start frozen for stability
    )
    model.to(device)
    # Freeze gen-only head (unused in raster training)
    if hasattr(model, "qwen") and hasattr(model.qwen, "lm_head"):
...
def save_val_samples_fixed(model, device, val_dataset: CustomDataset, indices: List[int], out_dir: str, epoch: int):
    ...
    with torch.no_grad():
        for i, idx in enumerate(indices, start=1):
            ...
            prompts = main_prompt.build_prompt(cond_t)
            rast = rast_t.unsqueeze(0).to(device)
            pred = model([img_t], prompts)  # [-1,1]
...
def train(cfg: Cfg):
    ...
    # Datasets / loaders
    train_ds = CustomDataset(cfg.train_img_dir, cfg.train_rast_dir, climate_dir=cfg.climate_json, with_cheating=False, climate_return="raw",
        image_return="pil")
    val_ds   = CustomDataset(cfg.val_img_dir, cfg.val_rast_dir, climate_dir=cfg.climate_json, with_cheating=False, climate_return="raw",
        image_return="pil")
    ...
    with open(os.path.join(cfg.out_dir, 'logs.out'), 'w') as f:
        for epoch in range(cfg.epochs):
            ...
            if is_main_process():
                ckpt_path = os.path.join(cfg.out_dir, 'ckpts', f"best.pt")

                if val_metrics['recon'] < best_val:
                    best_val = val_metrics['recon']
                    print("Saving best so far...")
...
def build_cfg_from_args():
    ...
    p.add_argument('--train_img_dir', type=str, default='.../small_dataset/satellite_images/train')
    p.add_argument('--train_rast_dir', type=str, default='.../small_dataset/normalised_risk_rasters/train')
    p.add_argument('--val_img_dir',   type=str, default='.../small_dataset/satellite_images/val')
    p.add_argument('--val_rast_dir',  type=str, default='.../small_dataset/normalised_risk_rasters/val')
    p.add_argument('--climate_json',  type=str, default='/work//wildfirerisk/small_dataset/climate_data.json')
    p.add_argument('--out_dir', type=str, default='/work/wildfirerisk/trainings/small/vlm_generator')
